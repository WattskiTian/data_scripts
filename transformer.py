...
def build_model(input_shape, num_classes):
    ...
    # 添加前馈网络
    feed_forward = tf.keras.layers.Dense(256, activation="relu")(attention_output)
    feed_forward = tf.keras.layers.Dense(128, activation="relu")(feed_forward)
    transformer_output = tf.keras.layers.LayerNormalization(epsilon=1e-6)(
        feed_forward + attention_output
    )

    # 池化操作（Global Average Pooling）
    pooled_output = tf.keras.layers.GlobalAveragePooling1D()(transformer_output)

    # 添加分类层
    outputs = tf.keras.layers.Dense(num_classes, activation="softmax")(pooled_output)
...
def main():
    ...
    log_dir = "./transformer_log"  # 指定日志保存目录
